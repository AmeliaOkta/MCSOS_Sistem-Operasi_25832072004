...
int main(void) {
    ...
    /* Fault injection tambahan (panduan SS18): nama terlalu panjang */
    const char long_name[] = "a_name_that_is_definitely_longer_than_27_bytes.txt";
    fails += expect_int("create-name-too-long",
        mcsfs1_create(&mnt, long_name), MCSFS1_ERR_NAMETOOLONG);

    /* Fault injection tambahan (panduan SS18): file terlalu besar */
    fails += expect_int("create-bigfile", mcsfs1_create(&mnt, "big.bin"), MCSFS1_ERR_OK);
    uint8_t oversize[4097];
    memset(oversize, 0xAA, sizeof(oversize));
    fails += expect_int("write-too-large",
        mcsfs1_write(&mnt, "big.bin", oversize, sizeof(oversize)), MCSFS1_ERR_RANGE);
